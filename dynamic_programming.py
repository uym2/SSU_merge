...
def max_of_cube(mapping,submap,input):
    ...
    L[1]=new_map(submap,(input[0],input[1]-1,input[2]-1))+new_map(mapping,(0,input[1],input[2]))
    L[2]=new_map(submap,(input[0]-1,input[1],input[2]-1))+new_map(mapping,(input[0],0,input[2]))
    L[3]=new_map(submap,(input[0]-1,input[1]-1,input[2]))+new_map(mapping,(input[0],input[1],0))
    L[4]=new_map(submap,(input[0],input[1],input[2]-1))+new_map(mapping,(0,0,input[2]))
    L[5]=new_map(submap,(input[0],input[1]-1,input[2]))+new_map(mapping,(0,input[1],0))
    L[6]=new_map(submap,(input[0]-1,input[1],input[2]))+new_map(mapping,(input[0],0,0))
    if max(L)==L[0]:
...
def dynamic_merge(struct):
    ...
    for i in range(len(dat_3_no_0)-1):
        if (dat_3_no_0[i+1][0]-dat_3_no_0[i][0])*(dat_3_no_0[i+1][1]-dat_3_no_0[i][1])*(dat_3_no_0[i+1][2]-dat_3_no_0[i][2])==1 and dat_3_no_0[i]!=[0,0,0]:
            path.append(dat_3_no_0[i])
	    path.append(dat_3_no_0[i+1])
        else:            
            submap,subpath=dynamic_program(dat_3_no_0[i],dat_3_no_0[i+1],mapping)
            path.extend(subpath)
    new_path=[]
